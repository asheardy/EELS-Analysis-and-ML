...
def min_max_normalize(data):
    min_val = np.min(data, axis=1).reshape((np.shape(data)[0],1))
    max_val = np.max(data, axis=1).reshape((np.shape(data)[0],1))
...
x_test = np.load('ML_data_x_test.npy').reshape((1369,1024)) 
...
history = model.fit(x_train, y_train_one_hot, batch_size = 128, epochs = 500, class_weight=dict(enumerate(class_weights)))
...
plt.imshow(y_pred, extent = [0, 76, 0, 76], interpolation = 'none')
plt.show()
